// foliage_system.js — M1 ITER 047 (ledger ITER_033, Gate C wiring).
...
// (position/scale from the RE-confirmed algorithm; heights sampled from the
// clean terrain chain). DETERMINISTIC: same inputs -> same render hash.
//
...
//        iter032 algorithm: FUN_0083a7d0 / FUN_0098fe00 / FUN_00990810 /
//        FUN_0095ac30 / FUN_0098cdf0 / FUN_0098ce30 / FUN_0095b180)
//     -> Three.js r185 WebGLRenderer
//     -> deterministic render + reproducible artifact
//
// ERA-BOUNDED PLACEHOLDERS (ALL LABELED — see PEFoliageCore.FOLIAGE_PLACEHOLDERS
// for the full list with the RE evidence):
//   [P-CLIMATE] climate index 0 (0.vcl) — the per-location climate input is
//       PLAUSIBLE-UNVERIFIED (the shared-selector hypothesis, iter032); the
//       DOCUMENTED CHOICE follows the established constant-byte-0 convention
//       of the terrain material page ([P1] in materials_confirmed.js); the
//       climate's elevation band union [-6, 1000] admits the region heights.
//   [P-WINDOW] the generation window = the proven region itself (the
//       historical grid extents are RNG/settings-scaled and not statically
//       pinneable — FUN_0098fe00's extent getters read runtime settings).
//   [P-UNITS] the GLB cache preserves the NIF centimeter units (the terrain
//       path scales m->cm x100, FUN_0082b790); the page bridges cm->m (0.01)
//       for the render. Bounding boxes are recorded per model in the census.
...
import { generateInstances, FOLIAGE_RE, FOLIAGE_PLACEHOLDERS } from '../src/peworld/PEFoliageCore.js';
...
// ---------- the generator (PEFoliageCore — the CONFIRMED chain) ----------
const POS_SCALE = 2.0;   // [P-POS-SCALE] _DAT_00a8c758 (u16 -> PE meters)
...
const windowU16 = {
  x0: Math.round(worldX0 * POS_SCALE), y0: Math.round(worldY0 * POS_SCALE),
  x1: Math.round(worldX1 * POS_SCALE), y1: Math.round(worldY1 * POS_SCALE),
};
...
// The instances — position/scale/model from the CONFIRMED algorithm; the
// height from the clean terrain chain; ROTATION = identity (RE-faithful: the
// spawn loop sets position + scale + model id only, iter032 bound 5).
...
for (const inst of instances) {
  ...
  const s = inst.scale * UNIT_BRIDGE;
...
const result = {
  p0: 'clean foliage chain: CONFIRMED iter032 algorithm -> generated instances -> deterministic render over the proven 9-tile region',
  chain: 'VegetationClimates.bnt + terrain.bnt bytes -> PESourceMount(BNT2) -> VCL climate records + TerrainTiles -> PEFoliageCore (FUN_0098fe00/00990810/0095ac30/0098cdf0/0098ce30/0095b180 semantics) -> r185 WebGLRenderer',
  reChain: FOLIAGE_RE,
  placeholders: { ...FOLIAGE_PLACEHOLDERS,
    ...
    'P-WINDOW': 'the generation window = the proven region (world x[worldX0..worldX1] y[worldY0..worldY1] -> u16 window); the historical grid extents are settings-scaled, not statically pinneable',
    'P-UNITS': 'GLB cache preserves NIF centimeters; render bridge 0.01 (m->cm x100 evidence: FUN_0082b790)',
  ...
  generator: {
    posScale: POS_SCALE, level: LEVEL, step: census.inputs.step,
    viewBand: VIEW_BAND, p3: P3, windowU16,
  },
  census: {
    ...census,
...
log(
`FOLIAGE SYSTEM page (ITER 047 / ledger ITER 033) - deterministic in-page: ${deterministic}
...
[P-CLIMATE] climate index ${CLIMATE_INDEX} (0.vcl) - the per-location climate input is PLAUSIBLE-UNVERIFIED (the shared-selector hypothesis, iter032); documented choice, NOT historical truth
[P-CELLSTREAM] the per-cell record CONTENT = local deterministic stand-in (the historical cell byte-stream origin NOT closed, iter032 bound 3); the record FORMAT {u16 x, u16 y, u32 id} + the spawn semantics are the CONFIRMED parts
[P-RNG-DIV] rand01 divisor = 32768.0 (the _DAT_00a7d7a8 runtime value not statically recoverable)
[P-RNG-P3] seed p3 = 0 (*(impl+0x24) UNVERIFIED); p2 = view band 10 (STRONGLY_SUPPORTED: the +0x2c 10/20/30 field pattern, FUN_0095b180)
[P-POS-SCALE] u16->world divisor = 2.0 (the _DAT_00a8c758 runtime value not statically recoverable; the u16 space then covers 65,536 half-units = a world up to 32,768, admitting the historical client coords)
[P-SCALE-FIELDS] lerp min/max = VCL col2/col3 (census min/max reading; field-level direction STRONGLY_SUPPORTED)
[P-WINDOW] the generation window = the proven region (the historical grid extents are settings-scaled)
[P-UNITS] GLB cache preserves NIF cm; render bridge 0.01 (FUN_0082b790 m->cm x100 evidence)
...
CONFIRMED from the binary (iter032, address-cited): the .vcl TSV parser FUN_0083a7d0 (12 values -> 0x30 records); the grid subdivision switch FUN_0098fe00 (1/2/4/8/16); the {u16,u16,u32} cell records FUN_00990810; the spawn loop FUN_0095b180 (pos = u16/K, scale = |lerp*2.0|, model id bind); the RNG FUN_0098cdf0 (position-hash seed) + FUN_0098ce30 (MSVC rand() 214013/2531011) + the sampler FUN_0095ac30; GetModel type-0x66 FUN_0094b1d0; rotation NOT FOUND in the spawn loop (identity = RE-faithful)`;
