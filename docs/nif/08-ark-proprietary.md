...
```
The 38 trailing bytes: UNKNOWN semantics (hash-kept raw).
...
```
Extension internals: UNKNOWN (raw-kept + SHA256). Max 121 B in passing corpus.

## NiArkShaderExtraData — 2,084×

```
ExtraData base + unknownInt i32 + unknownString SizedString
```
Confirmed by 3 independent external sources + 20/20 binary traces
(162–167 B consumption; 1,059/1,162 chain-exact). The string carries shader
effect config (semantics UNKNOWN; raw-kept).
...
### v10 variants:
Mode1 (u2=FFFFFFFF, u4=0): 0 B; Mode2 (u4=01000000): 33 B; G3A (u2=2,u3=0):
0 B; G3B: binary → boundary search; G3C: text-like → TEXT_CRLF grammar;
G3D (u3≠0, byte0=0x01 pure binary): size = byte[1]×5; G3E: boundary search.
