...
# STAGE GA8: transform-template closure - callers of FUN_00567770; decompile the
# template setters FUN_00730f60/f90/fb0/fd0; attribute readers FUN_00846840/
# FUN_00854720/FUN_004154f0; registration FUN_004148f0/FUN_00457930.
# Writes: 01_RAW\ghidra_output\GA8_PSEUDO_*.txt, GA8_CALLERS.json
...
TARGETS = [
    ...
    0x00846840, 0x00854720, 0x004154F0,
    0x004148F0, 0x00457930, 0x005F6990, 0x005F6850,
]
CALLER_QUERIES = {
    "FUN_00567770": 0x00567770,
    "FUN_00730f90": 0x00730F90,
    "FUN_00730fb0": 0x00730FB0,
...
def main():
    for va in TARGETS:
        ...
        if fn is None:
            continue
        c = decompile(fn)
        with open(os.path.join(OUT_RAW, "GA8_PSEUDO_%08X.txt" % fn.getEntryPoint().getOffset()), "w") as f:
            f.write("// body=%s\n" % ("0x%08X-0x%08X" % (
    ...
    with open(os.path.join(OUT_RAW, "GA8_CALLERS.json"), "w") as f:
        json.dump({"exec": EXEC, "callers": cq}, f, indent=2)

    print("GA8_DONE targets=%d" % len(TARGETS))
