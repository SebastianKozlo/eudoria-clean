...
- PRIMARY_EVIDENCE_PATHS:
    ...
    01_RAW/FUN_00437F70_DISASM.txt                   (Phase C: singleton getter; args ignored; SEH; census; 10 questions)
    01_RAW/FUN_0082B5A0_DISASM.txt                   (Phase D: scaled-subtract converter; bit-exact constants; x87 trace; 7 questions)
    01_RAW/SOURCE_VECTOR_LAYOUT_RAW.txt              (Phase B: +0x90 = m_kWorld translate; world vs local; vtable matrix)
    01_RAW/END_TO_END_VALUE_FLOW_RAW.txt              (Phase E: per-component formulas + S={0,0,0} never-written proof —
                                                      exhaustively re-measured in the correction-pass census
                                                      01_RAW/ORIGIN_TRIPLE_WRITE_CENSUS_RAW.txt, QC P1-1)
    01_RAW/NEGATIVE_CONTROL_RAW.txt                   (4 executed controls; FAILURE_CASE_DETECTED=YES)
    01_RAW/FALLBACK_PRIMARY_COMPARISON.txt            (five contract questions answered)
...
G4 PASS — 437F70 = origin-singleton getter (S={0,0,0} proven); plain ret; 99-caller census; 10 questions answered.
G5 PASS — 82B5A0 = out[i]=f32(f32(src[i]*K)-S[i]), K=(double)(float)0.01; exact x87 trace; 7 questions answered.
G6 PASS — per-component dataflow recovered; UNKNOWNs localized (space label; fallback stored-value scale).
...
G10 PASS — 4 executed negative controls; FAILURE_CASE_DETECTED=YES.
G11 STRONGLY_SUPPORTED — 100:1 conversion proven; absolute space label UNVERIFIED.
G12 PASS — identity permutation proven; axis labels UNVERIFIED statically.
G13 PASS — bit-exact 0.01 scale proven; cm->m reading = PLAUSIBLE interpretation.
G14 STRONGLY_SUPPORTED — position role corroborated by >=2 independent consumers; runtime trace not permitted.
G15/G16/G17 PENDING — governance (fresh QC; adjudication; persistence by pe-master-auditor).

## PRIMARY QUESTION — one-line answer at the highest status the bytes support
CONFIRMED: the primary path of SF slot3 does NOT copy the world translation — FUN_0050A050 writes nothing itself;
FUN_00437F70 fetches an immutable zero-vector origin singleton (never-written global triple), and FUN_0082B5A0
writes the caller's out float3 as the bit-exact per-component conversion
out[i] = f32( f32( m_kWorld.m_Translate[i] * (double)(float)0.01 ) - 0 ) = f32(W[i] * 0.01f) — a 100:1
engine->internal-space position conversion (identity axis order), paired with a measured inverse family
...
2. FUN_0082B5A0 is NOT a copy — the SF slot3 primary output is a 100:1-scaled position (engine translate x 0.01),
   so any consumer treating slot3 output as engine-units is off by 100x; the fallback output is a DIFFERENT
   derivation (raw stored placement value, no conversion) sharing only the float3 interface.
