...
#
# AMEND (correction run PE_935_SF_ORIGIN_MUTABILITY_CORRECTION_R1_20260915, executed in-place):
# the origin-singleton mutability claims of this report were CORRECTED — the original "S is never mutated
# after construction / no caller writes through the return / S = permanent zero vector" claims are RETRACTED
# (falsified by writer site 0x00458E27; the 'measured within 8 instructions over 99 sites' sentence was a
# fabricated measurement claim). Corrected status algebra: 02_ANALYSIS/ORIGIN_STATUS_CORRECTION.md;
# write-through census: 01_RAW/ORIGIN_SINGLETON_WRITE_THROUGH_CENSUS.csv; setter decode:
# 01_RAW/ORIGIN_SETTER_458D90_DISASM.txt. Measurement sections of this report stand; the superseded prose is
# amended inline below with AMEND markers. See 00_CONTROL/AMEND_LOG_R1.md AMEND-14+.
...
## ONE-LINE ANSWER (highest status the bytes support; AMENDED by the origin-mutability correction)
**CONFIRMED:** FUN_0050A050 itself writes NOTHING on the primary path; FUN_00437F70 is a lazy get-or-create of a
12-byte origin singleton S (initial value {0,0,0} — its source triple 0xba921c/20/24 is not written within the
enumerated channels of the correction-pass whole-image write census, which classifies all 268 imm32 occurrences
(loads/pushes/getters only) with 0 store-class candidates across every store encoding, 0 widened-overlap stores,
0 computed-base stores, all 24 push-site consumers and both getter chains read-only —
01_RAW/ORIGIN_TRIPLE_WRITE_CENSUS_RAW.txt — and it sits zero-initialized in .data's virtual tail; AMEND: the OLD
text asserted "(always {0,0,0})" — S's runtime mutability is a separate status: a writer site EXISTS (0x00458E27;
01_RAW/ORIGIN_SINGLETON_WRITE_THROUGH_CENSUS.csv), so ORIGIN_PERMANENT_ZERO = REJECTED and
ORIGIN_MUTATED_AT_RUNTIME/VALUE = UNVERIFIED — 02_ANALYSIS/ORIGIN_STATUS_CORRECTION.md), and FUN_0082B5A0 is the
sole writer of the caller's out float3, computing bit-exactly `out[i] = f32( f32( W[i] * (double)(float)0.01 ) -
S[i] )` per component (i = 0/4/8), i.e. the named NiAVObject's m_kWorld translate converted into a
K=0.01-scaled internal space with the base subtraction out = scale(W) - S (IF S == {0,0,0} — the measured initial
value — this is the pure x0.01 conversion and a 100:1 magnitude relation; AMEND: the unconditional "100:1-scaled
(world translate * 0.01, origin zero)" reading is retracted; identity axis order CONFIRMED) — with an exact
algebraic inverse family ((src+S)*100.0, FUN_0082B6A0) anchored on the same singleton.
...
3. FUN_00437F70 IGNORES both stack args (plain 'ret'; it is not a thiscall here), returns the singleton S from
    ...
    0xba921c/0xba9220/0xba9224). The triple is not written within the enumerated channels of the correction-pass
    whole-image write census (01_RAW/ORIGIN_TRIPLE_WRITE_CENSUS_RAW.txt: 268 imm32 occurrences containment-classified — 81+81+80
    ...
    consumers and both getter chains read-only; disclosed residuals enumerated in the census [T.10]) => S's
    INITIAL value is the zero vector. AMEND (origin-mutability correction): the OLD text continued "=> S = zero
    vector forever; S is never mutated after construction (no caller writes through the return within 8
    instructions; ...)" — RETRACTED: that claim was fabricated (no write-through measurement existed) and is
    falsified by writer site 0x00458E27 (three f32 stores through the returned pointer at offsets 0/4/8; setter
    FUN_00458D90 decode: 01_RAW/ORIGIN_SETTER_458D90_DISASM.txt; census: 01_RAW/ORIGIN_SINGLETON_WRITE_THROUGH_CENSUS.csv:
    99 boundary-verified sites, 1 writer, 43 read-only escapes, residuals disclosed). The 9 ECX-receiving functions
    read, forward or clobber ECX before use — none stores to [ECX+0/4/8] within their decoded windows (channel
    measurement, stands); the one apparent writer FUN_0082B790 clobbers ECX before use and writes only a fresh array
    (stands).
...
- H3 (FUN_0082B5A0 finalizes/mutates the primary-path result): CONFIRMED (stronger: it is the sole primary-path
  writer of out).
- H4 (output = named object's world position after deterministic conversion): CONFIRMED (out = f32(f32(world
  translate * 0.01) - S), identity axis order; IF S == {0,0,0} — the measured initial value — this is the pure
  x0.01 conversion; AMEND, origin-mutability correction: the unconditional "out = world_translate x 0.01" reading
  is retracted; the conversion itself is the deterministic part).
...
FUN_0082B6A0 ((src+S)*100.0) => byte-shape similarity is not operation identity; (4) mutability probe — AMEND
(origin-mutability correction): the ORIGINAL result "(4) ... 'dynamic origin (camera-relative)' reading REJECTED
(no writer exists ...; S is a permanent zero vector)" is SUPERSEDED: NEGATIVE_CONTROL_4 =
FALSIFIED_BY_COUNTEREXAMPLE (writer site 0x00458E27; the "no writer exists (99 sites measured)" premise was a
fabricated measurement claim). The historical Control 4 text is preserved byte-identical in
01_RAW/NEGATIVE_CONTROL_RAW.txt with an appended supersession block; Controls 1-3 STAND.
...
2. FUN_0082B5A0 is NOT a copy: it is a scaled-subtraction coordinate conversion; the SF slot3 primary output is
   scale(W) - S — IF S == {0,0,0} (the measured initial value) the output is numerically 100x smaller than the
   engine translate (0.01 scale, proven bit-exact; AMEND, origin-mutability correction: the unconditional "100x
   smaller" reading is retracted — S's runtime value is UNVERIFIED and a mutation channel exists).
...
## GATE SUMMARY (full table: 06_REPORT/STAGE_ACCEPTANCE_GATES.csv; AMENDED by the origin-mutability correction —
gate rows re-evaluated with correction-run evidence, row survival preserved)
...
measured; reads/writes/SEH/callee identities/census; 10 questions answered) — G4_EVIDENCE_CONSISTENCY RECHECKED
by the correction: FUN_00437F70_DISASM.txt was regenerated from the repaired generator (its false S-lifetime prose
removed); the measurement sections are byte-stable vs the pre-correction snapshot and the getter decode stands
(singleton get-or-create; EAX = S at return) — RECHECK PASS. G5 PASS (bit-exact constants; exact x87
per-component formulas; 7 questions answered — arithmetic decode untouched by the correction; K bits re-verified
bit-exact in 01_RAW/OUTPUT_FORMULA_REVALIDATION_RAW.txt). G6 PASS (per-component dataflow with UNKNOWNs localized;
basis corrected: the confirmed relation is out = scale(W) - S; the zero-origin simplification is a CONDITIONAL
special case, not the gate's unconditional basis). G7 PASS. G8 PASS (five fallback-vs-primary questions answered
explicitly). G9 PASS (5 context classes). G10 PASS (4 executed negative controls; FAILURE_CASE_DETECTED=YES) with
the CORRECTION SUPERSESSION recorded: NEGATIVE_CONTROL_4 = FALSIFIED_BY_COUNTEREXAMPLE (0x00458E27); Controls 1-3
STAND (supersession block in 01_RAW/NEGATIVE_CONTROL_RAW.txt). G11 STRONGLY_SUPPORTED (scale-by-0.01 conversion +
base subtraction proven; K bit-proven; the 100:1 magnitude relation is CONDITIONAL on S=={0,0,0}; absolute space
label UNVERIFIED). G12 PASS (identity permutation; labels UNVERIFIED statically). G13 PASS (bit-exact scale
re-verified; cm->m reading = PLAUSIBLE interpretation). G14 STRONGLY_SUPPORTED (position role corroborated by 2+
independent consumer classes — unchanged in kind; no unconditional scale relation asserted). G15 PASS / G16 PASS
(governance; historical MASTER_ACCEPTED adjudication stands as the historical record — the CURRENT scientific
adjudication for the corrected claims is the superseding review to be written by PE-MASTER; the gates row carries
the supersession-chain pointer) / G17 PASS (governance).
