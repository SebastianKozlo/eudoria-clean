...
# v3: sortowanie ORDINAL (Python sorted() = codepoint order), DFS preorder jak os.walk(topdown=True)
...
$script:totalFiles = 0
function Walk-Dir([string]$t) {
    $root = [System.IO.Path]::GetFullPath($t)
    # pliki biezacego katalogu, sortowanie ORDINAL po nazwie
    $fileNames = (Get-ChildItem -LiteralPath $t -File | ForEach-Object { $_.Name })
    $sorted = $fileNames | Sort-Object { $_ } -Culture ([System.Globalization.CultureInfo]::InvariantCulture)
    # porzadek ordinal: uzycie List[string].Sort(StringComparer.Ordinal)
    $flist = New-Object 'System.Collections.Generic.List[string]'
    foreach ($n in $fileNames) { [void]$flist.Add($n) }
    $flist.Sort([System.StringComparer]::Ordinal)
    foreach ($fn in $flist) {
        $p = Join-Path $t $fn
        $rel = [System.IO.Path]::GetFullPath($p).Substring($script:rootLen)
        $s = Get-FileSha256Upper $p
        $chunk = $rel + "|" + $s
        $bytes = $enc.GetBytes($chunk)
        $ms.Write($bytes, 0, $bytes.Length)
        $script:totalFiles++
    }
    $dlist = New-Object 'System.Collections.Generic.List[string]'
    foreach ($d in (Get-ChildItem -LiteralPath $t -Directory)) { [void]$dlist.Add($d.Name) }
    $dlist.Sort([System.StringComparer]::Ordinal)
    foreach ($dn in $dlist) { Walk-Dir (Join-Path $t $dn) }
}
$targets = @(
...
foreach ($t in $targets) {
    if (Test-Path -LiteralPath $t -PathType Leaf) {
        ...
        $before = $script:totalFiles
        Walk-Dir $t
        $cnt = $script:totalFiles - $before
        $dirTotals += ("{0} = {1}" -f (Split-Path $t -Leaf), $cnt)
...
$ms.Dispose()
$dirTotals | ForEach-Object { Write-Output ("DIR   {0}" -f $_) }
Write-Output ("TOTAL FILES: {0}" -f $script:totalFiles)
